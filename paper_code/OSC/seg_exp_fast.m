function [ E ] = seg_exp_fast(infile, save_dir, nclusters, max_iter)
	...
	%Hurricane Data
	%X = csvread('data/hurricane_matthew_normalized.csv',2);
	%X = csvread('data/hurricane_matthew_normalized_60min_sampling_interval.csv',2);
	%X = csvread('data/syn2_interp.csv',1);
	X = csvread(infile, 1);
	%X = csvread('data/Matthew_interp.csv',1);
	%X = csvread('data/hurricane_matthew_normalized_30min_sampling_interval.csv',2);
	...
	%S=csvread(strcat(save_dir, 's_matrix.txt')); 

	%nbCluster = 4;
	nbCluster = nclusters;

	%% OSC
	%maxIterationbCluster = 100;
	maxIterationbCluster = max_iter;
	lambda_1 = .5;
	beta_1 = 5; %controls S_exp
	beta_2 = 0.01;
	p = 1.1;
	diag = 1;
	...
	csvwrite(strcat(save_dir, 'temporal_segments.txt'),segmentindices); 
	...
	% hold on
	% Y = csvread('data/syn2_interp.csv',1);
