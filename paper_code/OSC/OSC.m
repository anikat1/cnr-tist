function [ Z, R, funVal, iteration ] = OSC( X, lambda_1, lambda_2, gamma_1, gamma_2, p, maxIterations, diagconstraint)
%% Problem
%
...
[~, xn, ~] = size(X); %xn indicates the number of timesteps.
...
%kron_R_Rt = kron(sparse(R*R'),speye(xn,xn));
%temp=X'*X;
%temp2=tril(temp); %convert temp into a lower triangular matrix.
%kron_Xt_X = kron(speye(xn,xn),sparse(temp2));
