...
function [] = demo_toy_seg_exog3_fast(resultsoutputdir,inputfile,laplacefile)
	...
	%Hyperparameter.
		...
		%{
		lam1=[-100];
        lam2=[-100,-70];%-30,-50, -100
		%lam2=[-50,0.5];
        %lam3=[300,500,1000];
		lam3=[250,200,270,300];
        beta=[1];
		%}
        nbclustersV=[3];
        %maxiter=[300];
    ...
    time_opt=0;
    time_normcut=0;
	timefile = strcat(resultsoutputdir,'scalability_timestep_500.csv');
                ...
                for lambda_3=lam3
                    for beta_1=beta
						...
						if funVal(it)>=0 && vInval==0
                            ...
                            et=toc;
                            time_normcut=et-st;
                            ttl_time=ttl_time+time_normcut;
                            fprintf(time,'%.4f\n',time_opt);
                            fprintf(time,'%.4f\n',time_normcut);
                            fprintf(time,'%.4f\n',ttl_time);
                            ...
                            fV=1;
                            %{
                            if fV & size(segmentindicesV,2)<=8
