...
function [] = demo_hurricane_seg_exog4_fast(resultsoutputdir,inputfile,visfile)
			    ...
			    for lambda_2=lam2
                    for lambda_3=lam3
                        for beta_1=beta
                            ...
                            tic;
                            %[U,S,V] = svd(X);
                            [V, U, funVal,it] = OSC_Exog3(X, L, 5, lambda_1, lambda_2, lambda_3, gamma_1, gamma_2, gamma_3, p,... 
                                             beta_1, maxIterationbCluster, diag, diag);
                            toc;
                            %fprintf('Iteration Number %d \n',it);
                            [oscVclusters,oscVeigenvectors,oscVeigenvalues] = ncutW(abs(V')*abs(V),nbClusterV); %normalized V cuts.
                            ...
                            s = sprintf('lam1 %.3f lam2 %.3f lam3 %.3f beta %.2f',lambda_1,lambda_2,lambda_3,beta_1);
                            disp(s);
                            ...
                            fV=1;
                            if fV
                                resultfilename=strcat('lam1_',mat2str(lambda_1),'_lam2_',mat2str(lambda_2),'lam3_',...
                                mat2str(lambda_3),'_clusV_',mat2str(nbClusterV),...
                                '_clusU_',mat2str(nbClusterU));  
