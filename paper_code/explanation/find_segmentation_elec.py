...
def find_segmentation(data_dir, clique, adj_file, filename, segfile, alpha, lmda, 
    save_dir, step, thres, nclusters, max_iter_seg, max_iter, save_file):
    """
    This is the version to run on the electricity datasets, where we need to generate a clique graph for the time series, and we already have a segmentation and we only need to find the corresponding explanation
...
if __name__ == '__main__':
    ...
    #save_dir = '../result/OSC_Hurricane_Results/harvey/' + non + '/'
    ...
    filename = sys.argv[2]#'Irma_60min_sample.csv'
    save_dir = sys.argv[3]#'../result/OSC_Hurricane_Results/irma/' + non + '/'
    adj_file = sys.argv[3]#'Irma.adjlist'
    segfile = sys.argv[4]#'segmentation.txt'
    ...
    clique = False
    find_segmentation(data_dir, clique, adj_file, filename, segfile, alpha, lmda, save_dir, step, thres, nclusters, max_iter_seg, max_iter, save_file)
